class Solution:
    ...
    def DFS(self, node):
        if self.visited[node] == -1: return False # cycle detected
        if self.visited[node] == 1: return True # has been finished, and been added to self.res
        self.visited[node] = -1 # mark as visited
        for x in self.graph[node]:
            if not self.DFS(x): return False
